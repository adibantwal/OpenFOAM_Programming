...
Description
    This solver explores debugging.
\*---------------------------------------------------------------------------*/
...
int main(int argc, char *argv[])
{
    ...
    while (runTime.loop())
    {
        ...
        // if runTime.timeName() is an integer print the p at the cell in the middle of the domain. Ideally it should be zero.

    if (mag( runTime.value() - round(runTime.value()) ) < 1e-6)
    {
        label midCell = mesh.C().size() / 2; // Index for the middle cell
        Info << "Pressure at the middle cell (" << midCell << "): " << p[midCell] << " at time = " << runTime.value() << nl << endl;
    };
